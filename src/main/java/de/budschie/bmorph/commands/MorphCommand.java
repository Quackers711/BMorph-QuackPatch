...
public class MorphCommand
{
	public static void registerCommands(CommandDispatcher<CommandSource> dispatcher)
	{
		...
		// CUSTOM QUACK711
		dispatcher.register(Commands.literal("removemorph")
				.requires(sender -> sender.hasPermissionLevel(2))
				.then(Commands.argument("player", EntityArgument.players()).executes(ctx -> {
							IMorphCapability cap = MorphUtil.getCapOrNull(ctx.getSource().asPlayer());
							int morphListSize;
							if (cap != null) {
								morphListSize = cap.getMorphList().getMorphArrayList().size();
							} else {
								morphListSize = 0;
							}
							return morphListSize;
						})
						.then(Commands.argument("index", IntegerArgumentType.integer(0)).executes(ctx ->
								{
									...
									if (cap != null && cap.getMorphList().getMorphArrayList().size() >= index) {
										cap.removeFromMorphList(index);
                                        ctx.getSource().sendFeedback(new StringTextComponent(TextFormatting.GREEN + "Successfully removed morph from list. "), true);
									}
									else {
										ctx.getSource().sendErrorMessage(new StringTextComponent(TextFormatting.RED + "Failed removing morph. Index might be out of range."));
									}
		...
		// CUSTOM QUACK711
		dispatcher.register(Commands.literal("morphlist")
				.requires(sender -> sender.hasPermissionLevel(2))
				.executes(ctx -> listMorphs(ctx.getSource(), ctx.getSource().asPlayer()))
				.then(Commands.argument("player", EntityArgument.player()).executes(ctx -> listMorphs(ctx.getSource(), EntityArgument.getPlayer(ctx, "player")))));
	}

	// CUSTOM QUACK711
	private static int listMorphs(CommandSource sender, PlayerEntity player) {
		IMorphCapability cap = MorphUtil.getCapOrNull(player);

		if (cap == null) {
			sender.sendErrorMessage(new StringTextComponent("The given player morph list could not be found."));
		}
		else if (cap.getMorphList().getMorphArrayList().isEmpty()) {
			sender.sendErrorMessage(new StringTextComponent("The given player does not have any morphs"));
		}
		else {
			StringBuilder builder = new StringBuilder(TextFormatting.GREEN + "The given player currently has following morphs: ").append(TextFormatting.WHITE);

			for(int i = 0; i < cap.getMorphList().getMorphArrayList().size(); i++)
			{
				MorphItem currentMorph = cap.getMorphList().getMorphArrayList().get(i);

				builder.append(currentMorph.getEntityType().getRegistryName().toString());

				if(i == (cap.getMorphList().getMorphArrayList().size() - 2))
				{
					builder.append(" and ");
				}
				else if(i != (cap.getMorphList().getMorphArrayList().size() - 1))
				{
					builder.append(", ");
				}
			}
